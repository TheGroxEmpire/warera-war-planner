...
class BuildProblem(Problem):
    def __init__(self, level, rank_bonus=1.0, pill_mode=False, pill_price=0.0, scaling_mode='dev', health_scaling='prod', arm_step=6, ddg_step=5, hp_step=15, food_step=10, overflow_multiplier=1.0, base_hp=None, base_hun=None):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for i in range(n_pop):
            ...
            if g_idx not in self._gear_cache:
                ...
                combined_baseline, _ = apply_gear_to_baseline(gear_choice, base_hp=self.base_hp, base_hun=self.base_hun)
                self._gear_cache[g_idx] = make_skill_tables(combined_baseline, scaling_mode=self.scaling_mode, health_scaling=self.health_scaling, arm_step=self.arm_step, ddg_step=self.ddg_step, hp_step=self.hp_step)
...
def optimize_max_damage(level, rank_bonus=1.0, pill_mode=False, pill_price=0.0, scaling_mode='dev', health_scaling='prod', arm_step=6, ddg_step=5, hp_step=15, food_step=10, overflow_multiplier=1.0, base_hp=None, base_hun=None):
    """Run a focused single-objective optimization to find the max-damage build."""
    problem = MaxDamageProblem(level, rank_bonus=rank_bonus, pill_mode=pill_mode, pill_price=pill_price, scaling_mode=scaling_mode, health_scaling=health_scaling, arm_step=arm_step, ddg_step=ddg_step, hp_step=hp_step, food_step=food_step, overflow_multiplier=overflow_multiplier, base_hp=base_hp, base_hun=base_hun)
    algorithm = NSGA2(
        pop_size=100,
    ...
    total_damage, total_cost, diag = compute_totals(
        ...
        tables=problem._tables, scaling_mode=scaling_mode, health_scaling=health_scaling, arm_step=arm_step, ddg_step=ddg_step, hp_step=hp_step, food_step=food_step, overflow_multiplier=overflow_multiplier, base_hp=base_hp, base_hun=base_hun
    )
...
def optimize_worker(args):
    level, seed, rank_bonus, pill_mode, pill_price, scaling_mode, health_scaling, arm_step, ddg_step, hp_step, food_step, overflow_multiplier, base_hp, base_hun = args
    problem = BuildProblem(level, rank_bonus=rank_bonus, pill_mode=pill_mode, pill_price=pill_price, scaling_mode=scaling_mode, health_scaling=health_scaling, arm_step=arm_step, ddg_step=ddg_step, hp_step=hp_step, food_step=food_step, overflow_multiplier=overflow_multiplier, base_hp=base_hp, base_hun=base_hun)
...
def optimize(level, verbose=True, rank_bonus=1.20, pill_mode=False, pill_price=0.0, scaling_mode='dev', health_scaling='prod', arm_step=6, ddg_step=5, hp_step=15, food_step=10, overflow_multiplier=1.0, base_hp=None, base_hun=None):
    ...
    args = [(level, int(seeds[i]), rank_bonus, pill_mode, pill_price, scaling_mode, health_scaling, arm_step, ddg_step, hp_step, food_step, overflow_multiplier, base_hp, base_hun) for i in range(num_runs)]
