...
class BuildProblem(Problem):
    def __init__(self, level, rank_bonus=1.0, pill_mode=False, pill_price=0.0, scaling_mode='dev', health_scaling='prod', hp_step=15, bread_pct=10, steak_pct=20, fish_pct=30, overflow_multiplier=1.0, base_hp=None, base_hun=None, armor_gear_multiplier=1.0):
        ...
        self.bread_pct = bread_pct
        self.steak_pct = steak_pct
        self.fish_pct = fish_pct
        self.overflow_multiplier = overflow_multiplier
        self.base_hp = base_hp
        self.base_hun = base_hun
        self.armor_gear_multiplier = armor_gear_multiplier
        self._gear_cache = {}
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for i in range(n_pop):
            ...
            if g_idx not in self._gear_cache:
                ...
                combined_baseline, _ = apply_gear_to_baseline(gear_choice, base_hp=self.base_hp, base_hun=self.base_hun, armor_gear_multiplier=self.armor_gear_multiplier)
                self._gear_cache[g_idx] = make_skill_tables(combined_baseline, scaling_mode=self.scaling_mode, health_scaling=self.health_scaling, hp_step=self.hp_step)
            ...
            total_damage, total_cost, _ = compute_totals(
                ...
                tables=self._gear_cache[g_idx], scaling_mode=self.scaling_mode, health_scaling=self.health_scaling, hp_step=self.hp_step, bread_pct=self.bread_pct, steak_pct=self.steak_pct, fish_pct=self.fish_pct, overflow_multiplier=self.overflow_multiplier, armor_gear_multiplier=self.armor_gear_multiplier
            )
            F[i, 0] = -total_damage
            F[i, 1] = total_cost
...
class MaxDamageProblem(Problem):
    ...
    def __init__(self, level, rank_bonus=1.0, pill_mode=False, pill_price=0.0, scaling_mode='dev', health_scaling='prod', hp_step=15, bread_pct=10, steak_pct=20, fish_pct=30, overflow_multiplier=1.0, base_hp=None, base_hun=None, armor_gear_multiplier=1.0):
        ...
        self._tables = make_skill_tables(combined_baseline, scaling_mode=scaling_mode, health_scaling=health_scaling, hp_step=hp_step)
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for i in range(n_pop):
            total_damage, _, _ = compute_totals(
                ...
                rank_bonus=self.rank_bonus, pill_mode=self.pill_mode, pill_price=self.pill_price,
                tables=self._tables, scaling_mode=self.scaling_mode, health_scaling=self.health_scaling, hp_step=self.hp_step, bread_pct=self.bread_pct, steak_pct=self.steak_pct, fish_pct=self.fish_pct, overflow_multiplier=self.overflow_multiplier, armor_gear_multiplier=self.armor_gear_multiplier
            )
            F[i, 0] = -total_damage
...
def optimize_max_damage(level, rank_bonus=1.0, pill_mode=False, pill_price=0.0, scaling_mode='dev', health_scaling='prod', hp_step=15, bread_pct=10, steak_pct=20, fish_pct=30, overflow_multiplier=1.0, base_hp=None, base_hun=None, armor_gear_multiplier=1.0):
    """Run a focused single-objective optimization to find the max-damage build."""
    problem = MaxDamageProblem(level, rank_bonus=rank_bonus, pill_mode=pill_mode, pill_price=pill_price, scaling_mode=scaling_mode, health_scaling=health_scaling, hp_step=hp_step, bread_pct=bread_pct, steak_pct=steak_pct, fish_pct=fish_pct, overflow_multiplier=overflow_multiplier, base_hp=base_hp, base_hun=base_hun, armor_gear_multiplier=armor_gear_multiplier)
    algorithm = NSGA2(
        pop_size=100,
    ...
    total_damage, total_cost, diag = compute_totals(
        ...
        tables=problem._tables, scaling_mode=scaling_mode, health_scaling=health_scaling, hp_step=hp_step, bread_pct=bread_pct, steak_pct=steak_pct, fish_pct=fish_pct, overflow_multiplier=overflow_multiplier, base_hp=base_hp, base_hun=base_hun, armor_gear_multiplier=armor_gear_multiplier
    )
...
def optimize_worker(args):
    level, seed, rank_bonus, pill_mode, pill_price, scaling_mode, health_scaling, hp_step, bread_pct, steak_pct, fish_pct, overflow_multiplier, base_hp, base_hun, armor_gear_multiplier = args
    problem = BuildProblem(level, rank_bonus=rank_bonus, pill_mode=pill_mode, pill_price=pill_price, scaling_mode=scaling_mode, health_scaling=health_scaling, hp_step=hp_step, bread_pct=bread_pct, steak_pct=steak_pct, fish_pct=fish_pct, overflow_multiplier=overflow_multiplier, base_hp=base_hp, base_hun=base_hun, armor_gear_multiplier=armor_gear_multiplier)
...
def optimize(level, verbose=True, rank_bonus=1.20, pill_mode=False, pill_price=0.0, scaling_mode='dev', health_scaling='prod', hp_step=15, bread_pct=10, steak_pct=20, fish_pct=30, overflow_multiplier=1.0, base_hp=None, base_hun=None, armor_gear_multiplier=1.0):
    ...
    args = [(level, int(seeds[i]), rank_bonus, pill_mode, pill_price, scaling_mode, health_scaling, hp_step, bread_pct, steak_pct, fish_pct, overflow_multiplier, base_hp, base_hun, armor_gear_multiplier) for i in range(num_runs)]
